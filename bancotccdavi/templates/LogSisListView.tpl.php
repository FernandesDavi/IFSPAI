<?php
	$this->assign('title','BANCOTCCDAVI | LogSises');
...
<h1>
	<i class="icon-th-list"></i> LOGs do Sistema
	...
	<script type="text/template" id="logSisCollectionTemplate">
		...
		<thead>
			<tr>
				<th id="header_IdLog">Id Log<% if (page.orderBy == 'IdLog') { %> <i class='icon-arrow-<%= page.orderDesc ? 'up' : 'down' %>' /><% } %></th>
		...
		<% items.each(function(item) { %>
			<tr id="<%= _.escape(item.get('idLog')) %>">
				<td><%= _.escape(item.get('idLog') || '') %></td>
	...
	<script type="text/template" id="logSisModelTemplate">
		<form class="form-horizontal" onsubmit="return false;">
			<fieldset>
				<div id="idLogInputContainer" class="control-group">
					<label class="control-label" for="idLog">Id Log</label>
					<div class="controls inline-inputs">
						<span class="input-xlarge uneditable-input" id="idLog"><%= _.escape(item.get('idLog') || '') %></span>
